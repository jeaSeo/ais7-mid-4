...
fig= px.scatter(raw02[(raw02["지출목적_x"].isin(["식료품 및 비주류음료", "주택 수도 전기 및 연료","보건"
            ,"통신", "교육", "음식 및 숙박", "의류 및 신발"])) & (raw02["소득계층"] != "전체")]
        , x = "소비자물가지수", y = "가계지출", color = "지출목적_x"
...
hm_fig = px.imshow(pv, text_auto=True, color_continuous_scale='magenta')
...
#heatmap 시각화
hm_fig = px.imshow(hs_he_pv, text_auto=True, color_continuous_scale='magenta')
hm_fig.update_layout(
    width= 1000,
    height= 400,
    # , margin=dict(l=20, r=0, t=10, b=0)
...
hm_fig = px.imshow(ct_he_pv, text_auto=True, color_continuous_scale='magenta')
hm_fig.update_layout(
    width= 1000,
    height= 800,
    # , margin=dict(l=20, r=0, t=10, b=0)
